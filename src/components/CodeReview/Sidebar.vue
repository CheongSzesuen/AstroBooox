<template>
  <aside :class="sidebarClass">
    ...
    <div v-else-if="pullRequests.length === 0" class="flex flex-1 flex-col items-center justify-center gap-3 px-2 text-center text-sm text-muted-foreground">
    ...
    <button
      type="button"
      class="mt-2 inline-flex w-full items-center justify-between rounded-md border border-border bg-muted/40 px-2.5 py-2 text-sm text-muted-foreground transition-colors hover:bg-accent"
      @click="$emit('toggle')"
    >
...
const sidebarClass = computed(() => [
  ...
  props.isCollapsed
    ...
    : 'left-3 top-[4rem] bottom-3 w-[17.25rem] rounded-xl p-2.5 md:left-4 md:top-[4.25rem] md:bottom-4 md:w-80 md:p-3'
])
