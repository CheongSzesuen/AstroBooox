    ...
    <div class="Layout-sidebar">
      ...
      <nav aria-label="File Tree Navigation">
        <ul class="ActionList ActionList--tree ActionList--full" role="tree" aria-label="File Tree">
          <template v-for="file in filteredFiles" :key="file.filename">
            <!-- 文件夹行（针对非根目录文件） -->
            <li 
              v-if="!isRootFile(file.filename) && shouldShowFolder(file)"
              class="ActionList-item js-tree-node"
              role="treeitem"
            >
              <div class="ActionList-content hx_ActionList-content" @click="toggleFolder(getFolderPath(file.filename))">
                <span class="ActionList-item-action ActionList-item-action--leading">
                  <svg 
                    ...
                    :class="{ 'rotate-90': !isFolderOpen(getFolderPath(file.filename)) }"
                  >
              ...
              <div class="ActionList-content hx_ActionList-content">
                ...
                <span class="ActionList-item-visual ActionList-item-visual--trailing">
                  <svg 
                    ...
                    width="16" 
                    class="octicon"
                    :style="getStatusIconColor(file.status)"
                  >
                    <path v-if="file.status === 'added'" d="M2.75 1h10.5c.966 0 1.75.784 1.75 1.75v10.5A1.75 1.75 0 0 1 13.25 15H2.75A1.75 1.75 0 0 1 1 13.25V2.75C1 1.784 1.784 1 2.75 1Zm10.5 1.5H2.75a.25.25 0 0 0-.25.25v10.5c0 .138.112.25.25.25h10.5a.25.25 0 0 0 .25-.25V2.75a.25.25 0 0 0-.25-.25ZM8 4a.75.75 0 0 1 .75.75v2.5h2.5a.75.75 0 0 1 0 1.5h-2.5v2.5a.75.75 0 0 1-1.5 0v-2.5h-2.5a.75.75 0 0 1 0-1.5h2.5v-2.5A.75.75 0 0 1 8 4Z"></path>
                    <path v-else-if="file.status === 'removed'" d="M13.25 1c.966 0 1.75.784 1.75 1.75v10.5A1.75 1.75 0 0 1 13.25 15H2.75A1.75 1.75 0 0 1 1 13.25V2.75C1 1.784 1.784 1 2.75 1ZM2.75 2.5a.25.25 0 0 0-.25.25v10.5c0 .138.112.25.25.25h10.5a.25.25 0 0 0 .25-.25V2.75a.25.25 0 0 0-.25-.25Zm8.5 6.25h-6.5a.75.75 0 0 1 0-1.5h6.5a.75.75 0 0 1 0 1.5Z"></path>
                    <path v-else-if="file.status === 'modified'" d="M13.25 1c.966 0 1.75.784 1.75 1.75v10.5A1.75 1.75 0 0 1 13.25 15H2.75A1.75 1.75 0 0 1 1 13.25V2.75C1 1.784 1.784 1 2.75 1ZM2.75 2.5a.25.25 0 0 0-.25.25v10.5c0 .138.112.25.25.25h10.5a.25.25 0 0 0 .25-.25V2.75a.25.25 0 0 0-.25-.25ZM8 10a2 2 0 1 1-.001-3.999A2 2 0 0 1 8 10Z"></path>
                    <path v-else-if="file.status === 'renamed'" d="M13.25 1c.966 0 1.75.784 1.75 1.75v10.5A1.75 1.75 0 0 1 13.25 15H2.75A1.75 1.75 0 0 1 1 13.25V2.75C1 1.784 1.784 1 2.75 1ZM2.75 2.5a.25.25 0 0 0-.25.25v10.5c0 .138.112.25.25.25h10.5a.25.25 0 0 0 .25-.25V2.75a.25.25 0 0 0-.25-.25Zm9.03 6.03-3.25 3.25a.749.749 0 0 1-1.275-.326.749.749 0 0 1 .215-.734l1.97-1.97H4.75a.75.75 0 0 1 0-1.5h4.69L7.47 5.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018l3.25 3.25a.75.75 0 0 1 0 1.06Z"></path>
                  </svg>
                </span>
              </div>
            </li>
...
import type { FilesTabContentProps } from '@/type/codeReview'
...
// 判断是否需要显示文件夹行
const shouldShowFolder = (file: any) => {
  const folderPath = getFolderPath(file.filename)
  // 如果是第一个该文件夹下的文件，则显示文件夹行
  return filteredFiles.value.findIndex(f => getFolderPath(f.filename) === folderPath) === filteredFiles.value.indexOf(file)
}

// 判断是否需要显示文件
const shouldShowFile = (file: any) => {
  if (isRootFile(file.filename)) return true
  const folderPath = getFolderPath(file.filename)
  return isFolderOpen(folderPath)
}
...
const filteredFiles = computed(() => {
  if (!searchQuery.value) return props.changedFiles
  return props.changedFiles.filter(file => 
...
// Get file depth for indentation
const getFileDepth = (filename: string) => {
  return filename.split('/').length - 1 // 从0开始计算深度
}
